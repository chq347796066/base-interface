 ...
 * @Desc类说明: 项目立项总部质控审核连线监听器
 */
@Service()
@Slf4j
public class MainQualityTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("MainQualityTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcmQualityAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("MainQualityTaskListener businessId:" + businessId + ",nodeId:pcmQualityAuditLine,businessType:"
