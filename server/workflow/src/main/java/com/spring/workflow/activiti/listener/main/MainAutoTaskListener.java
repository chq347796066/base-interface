 ...
 * @Desc类说明: 项目立项总部自动化室审核连线监听器
 */
@Service()
@Slf4j
public class MainAutoTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("MainAutoTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcmAutoAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("MainAutoTaskListener businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
