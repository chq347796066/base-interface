 ...
 * @Desc类说明: 项目立项分厂单位领导审核连线监听器
 */
@Service()
@Slf4j
public class SubDirChiefTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("SubDirChiefTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcsDirChiefAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("SubDirChiefTaskListener businessId:" + businessId + ",nodeId:pcsDirChiefAuditLine,businessType:"
