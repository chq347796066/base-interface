 ...
 * @Desc类说明: 项目立项分厂自动化室审核连线监听器
 */
@Service()
@Slf4j
public class SubAutoTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("SubAutoTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcsAutoAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("SubAutoTaskListener businessId:" + businessId + ",nodeId:pcsAutoAuditLine,businessType:"
