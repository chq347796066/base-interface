 ...
 * @Desc类说明: 项目立项分厂信息化审核连线监听器
 */
@Service()
@Slf4j
public class SubInfoTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("SubInfoTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcsInfoAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("SubInfoTaskListener businessId:" + businessId + ",nodeId:pcsInfoAuditLine,businessType:"
