 ...
 * @Desc类说明: 项目立项分厂工艺审核连线监听器
 */
@Service()
@Slf4j
public class SubWorkTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("SubWorkTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcsWorkAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("SubWorkTaskListener businessId:" + businessId + ",nodeId:pcsWorkAuditLine,businessType:"
