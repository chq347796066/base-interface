 ...
 * @Desc类说明: 项目立项总部生产审核连线监听器
 */
@Service()
@Slf4j
public class MainProductionTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("MainProductionTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcmProductAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("MainProductionTaskListener businessId:" + businessId + ",nodeId:pcmProductAuditLine,businessType:"
