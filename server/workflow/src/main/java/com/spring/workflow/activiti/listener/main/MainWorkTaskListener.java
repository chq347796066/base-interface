 ...
 * @Desc类说明: 项目立项总部工艺审核连线监听器
 */
@Service()
@Slf4j
public class MainWorkTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("MainWorkTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcmWorkAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("MainWorkTaskListener businessId:" + businessId + ",nodeId:pcmWorkAuditLine,businessType:"
