 ...
 * @Desc类说明: 项目立项总部智能化领导审核连线监听器
 */
@Service()
@Slf4j
public class MainCleverTaskListener implements TaskListener, JavaDelegate {
	@Autowired
	...
	public void execute(DelegateExecution execution) throws Exception {
		...
		log.info("MainCleverTaskListener begin businessId:" + businessId + ",nodeId:pcmAutoAuditLine,businessType:"
				+ businessType);
		...
		params.put("nodeId", "pcmCleverAuditLine");
		// 调用存储过程动态获取审核用户列表
		...
		log.info("MainCleverTaskListener businessId:" + businessId + ",nodeId:pcmCleverAuditLine,businessType:"
