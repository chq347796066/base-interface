...
@Aspect
@Component
public class TenantPagePower {

    //切入点
    @Pointcut("@annotation(com.spring.common.annotation.TenantPagePower)")
    ...
    private Object handleEntity(Class clazz, Object args) throws Exception {
        ...
        if(!Constants.SYSADMIN.equals(RequestUtils.getUserId())) {
            if (!StringUtils.isEmpty(RequestUtils.getTenantId())) {
                ...
                log.info("TenantDataPower,tenantId:"+RequestUtils.getTenantId());
            }
        }
        return newClazz;
    }
