...
public class DateUtil {
	 ...
	 * @return
	 */
	public static String getFristDayOfCurrentMonth(Date date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
	...
	public static String getLastDayAfterMonths(String day, int cycle) {
		...
		return DateUtil.getLastDayOfMonth(calendar.getTime());
	}
	...
	public static List<String> getMonthBetween(String minDate, String maxDate) throws Exception {
		...
		min.setTime(DateUtil.parseDate(minDate));
		min.set(min.get(Calendar.YEAR), min.get(Calendar.MONTH), 1);

		Calendar max = Calendar.getInstance();
		max.setTime(DateUtil.parseDate(maxDate));
		max.set(max.get(Calendar.YEAR), max.get(Calendar.MONTH), 2);

		Calendar curr = min;
		while (curr.before(max)) {
			result.add(DateUtil.formatDate(DateUtil.DATE_FORMAT_DAY_YEAR_II, curr.getTime()));
	...
	public static void main(String[] args) throws Exception {
		System.out.println("获取第一天："+getFristDayOfCurrentMonth(new Date()));
